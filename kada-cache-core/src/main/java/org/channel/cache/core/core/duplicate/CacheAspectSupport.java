 ...
 */
@Slf4j
public abstract class CacheAspectSupport extends org.channel.cache.core.core.duplicate.AbstractCacheInvoker
    ...
    private final org.channel.cache.core.core.duplicate.CacheOperationExpressionEvaluator evaluator = new org.channel.cache.core.core.duplicate.CacheOperationExpressionEvaluator();
    ...
    private Object execute(final CacheOperationInvoker invoker, Method method, CacheOperationContexts contexts) {
        ...
        if (contexts.isSynchronized()) {
            ...
            if (isConditionPassing(context, org.channel.cache.core.core.duplicate.CacheOperationExpressionEvaluator.NO_RESULT)) {
                Object key = generateKey(context, org.channel.cache.core.core.duplicate.CacheOperationExpressionEvaluator.NO_RESULT);
        ...
        processCacheEvicts(contexts.get(CacheEvictOperation.class), true,
                org.channel.cache.core.core.duplicate.CacheOperationExpressionEvaluator.NO_RESULT);
        ...
        if (cacheHit == null) {
            collectPutRequests(contexts.get(CacheableOperation.class),
                    org.channel.cache.core.core.duplicate.CacheOperationExpressionEvaluator.NO_RESULT, cachePutRequests);
        }
    ...
    private boolean hasCachePut(CacheOperationContexts contexts) {
        ...
        for (CacheOperationContext context : cachePutContexts) {
            try {
                if (!context.isConditionPassing(org.channel.cache.core.core.duplicate.CacheOperationExpressionEvaluator.RESULT_UNAVAILABLE)) {
    ...
    private void performCacheEvict(
        ...
        for (Cache cache : context.getCaches()) {
            if (operation.isCacheWide()) {
                ...
                doClear(cache,context);
            } else {
                if (key == null) {
                    key = generateKey(context, result);
                }
                logInvalidating(context, operation, key);
                doEvict(cache, key,context);
            }
        }
    }
    ...
    private Cache.ValueWrapper findCachedItem(Collection<CacheOperationContext> contexts) {
        Object result = org.channel.cache.core.core.duplicate.CacheOperationExpressionEvaluator.NO_RESULT;
        for (CacheOperationContext context : contexts) {
    ...
    private Cache.ValueWrapper findInCaches(CacheOperationContext context, Object key) {
        for (Cache cache : context.getCaches()) {
            Cache.ValueWrapper wrapper = doGet(cache, key,context);
            if (wrapper != null) {
                if (logger.isTraceEnabled()) {
        ...
        public void apply(@Nullable Object result) {
            if (this.context.canPutToCache(result)) {
                for (Cache cache : this.context.getCaches()) {
                    doPut(cache, this.key, result,context);
                }
            }
        }
    }
